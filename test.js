...
for (const p of PROVIDERS) {
  ...
  assert(
    Array.isArray(PROVIDER_MODELS[p]) && PROVIDER_MODELS[p].length > 0,
    `model list exists for ${p}`,
  );
  assert(
    PROVIDER_MODELS[p].includes(DEFAULT_MODELS[p]),
    `default is in model list for ${p}`,
  );
}
...
assert(
  parseSSE(evSources.trim())?.sources.length === 2,
...
function buildChatHistory(messages) {
  ...
  return messages.slice(-10).filter((m) => !m.loading && m.content.trim());
}

const msgs = [
  { role: "user", content: "What is this doc about?", loading: false },
  { role: "assistant", content: "It is about RAG pipelines.", loading: false },
  { role: "user", content: "Tell me more", loading: false },
];
const history = buildChatHistory(msgs);
assert(history.length === 3, "history has 3 messages");
assert(history[0].role === "user", "first message is user");
assert(history[1].role === "assistant", "second message is assistant");
assert(!history.some((m) => m.loading), "no loading messages in history");

// test 10-message limit
...
assert(
  buildChatHistory(longHistory).length === 10,
  "history capped at 10 messages",
);

// ── 9. Suggested questions format ─────────────────────────────────────────────
console.log("\n suggested questions");
...
assert(
  parseSuggestedQuestions('["Q1?", "Q2?", "Q3?"]').length === 3,
  "3 questions parsed",
);
assert(
  parseSuggestedQuestions('["Q1?", "Q2?", "Q3?", "Q4?"]').length === 3,
  "capped at 3",
);
assert(parseSuggestedQuestions("not json").length === 0, "bad JSON → empty");
assert(parseSuggestedQuestions("").length === 0, "empty → empty");
assert(
  parseSuggestedQuestions('["Only one"]')[0] === "Only one",
