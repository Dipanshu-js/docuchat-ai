...
export function ChatWindow({ collectionId, files, onDelete }: ChatWindowProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: uid(),
      role: "assistant",
      content: `Ready! Loaded ${files.length === 1 ? `"${fileNames[0]}"` : `${files.length} files`}.\n\nAsk me anything about ${files.length === 1 ? "it" : "them"}.`,
    },
  ]);
    ...
    const fetchSuggestions = async () => {
      try {
        const res = await fetch("/api/suggest", {
          ...
          body: JSON.stringify({ collectionId, fileNames, provider, model }),
        });
  ...
  const sendMessage = useCallback(
    async (text?: string) => {
      ...
      // Build history from non-loading messages (exclude welcome message)
      const history = messages
        .filter((m) => !m.loading && m.id !== messages[0].id)
    ...
    [input, loading, messages, collectionId, provider, model],
  );
    ...
    setMessages([
      {
        id: uid(),
        role: "assistant",
        content: `Chat cleared. Still loaded: ${fileNames.join(", ")}. Ask me anything.`,
      },
    ]);
  };
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Provider bar */}
      <div className="flex items-center gap-2 px-4 py-2 border-b border-zinc-800 bg-zinc-900/60 flex-wrap">
        ...
        <select
          ...
          className="bg-zinc-800 text-zinc-300 text-xs rounded-lg px-2 py-1.5
        ...
        <button
          onClick={clearChat}
          className="ml-auto text-zinc-600 hover:text-zinc-400 text-xs flex items-center
